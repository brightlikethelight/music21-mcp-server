...
from music21_mcp.auth.models import (AccessToken, AuthorizationRequest,
                                     ClientRegistration, ClientType, GrantType,
                                     RefreshToken, ResponseType, TokenRequest,
                                     User, UserSession)
from music21_mcp.auth.oauth2_provider import OAuth2Config, OAuth2Provider
from music21_mcp.auth.security import (SecurityMiddleware,
                                       generate_code_challenge,
                                       generate_code_verifier,
                                       verify_code_challenge)
from music21_mcp.auth.session_manager import SessionConfig, SessionManager
from music21_mcp.auth.storage import (InMemoryOAuth2Storage,
                                      InMemorySessionStorage)

# Skip MCP integration tests if MCP is not available
try:
    from music21_mcp.auth.mcp_integration import (AuthenticatedMCPServer,
                                                  MCPAuthContext,
                                                  validate_mcp_token)
                    ...
                    tools.append(
                        {
                            ...
                            "required_scope": self.tool_permissions.get(
                                tool_name, "read"
                            ),
                        }
                    )
...
async def session_manager(session_storage):
    ...
    config = SessionConfig(session_ttl_minutes=30, max_sessions_per_user=5)
...
class TestOAuth2Models:
    ...
    def test_user_session_refresh(self):
        ...
        session = UserSession(
            user_id="user-123", expires_at=datetime.utcnow() + timedelta(minutes=30)
        )
...
class TestSessionManager:
    ...
    async def test_session_data_update(self, session_manager, test_user):
        ...
        session = await session_manager.create_session(
            user=test_user, data={"initial": "value"}
        )

        # Update data
        await session_manager.update_session_data(
            session.session_id, {"new": "data", "count": 42}
        )
