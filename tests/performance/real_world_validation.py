...
class RealWorldValidator:
    ...
    def __init__(self, test_corpus_dir: str = "test_corpus/imslp"):
        ...
        self.test_suites = {
            'basic_analysis': [
                'import', 'key_analysis', 'chord_analysis', 'score_info'
            ],
            'advanced_analysis': [
                'harmony_analysis', 'voice_leading', 'pattern_recognition'
            ],
            'generation': [
                'harmonization', 'counterpoint', 'style_imitation'
            ],
            'io_operations': [
                'export', 'list', 'delete'
            ]
    ...
    async def validate_file(self, file_path: Path, file_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if import_result['status'] != 'success':
                ...
                file_result['errors']['import'] = import_result.get('message', 'Import failed')
                ...
                try:
                    ...
                    if tool_name == 'export':
                        ...
                        if score and len(score.parts) == 1:
                            ...
                            result = {'status': 'skipped', 'reason': 'Not suitable for counterpoint'}
                    ...
                    if result.get('status') == 'success' or tool_name == 'list':
                        ...
                        file_result['errors'][tool_name] = result.get('message', 'Unknown error')
                    ...
                    logger.error(f"Tool {tool_name} failed on {file_path.name}: {str(e)}")
    ...
    def generate_compatibility_matrix(self) -> str:
                ...
                for file_path, file_result in self.results['file_results'].items():
                    if tool_name in file_result['errors']:
                        ...
                        if "Exception:" in error_msg:
                            error_type = error_msg.split("Exception:")[1].split(":")[0].strip()
                        else:
        ...
        for period, stats in sorted(period_stats.items()):
            if stats['total'] > 0:
                ...
                report.append(f"  {period.capitalize()}: {failure_rate:.1f}% failure rate ({stats['failed']}/{stats['total']})")
        ...
        for tool_name, durations in sorted(tool_performance.items()):
            if durations:
                ...
                report.append(f"  {tool_name}: {avg_time:.3f}s avg, {max_time:.3f}s max")
        ...
        if self.results['success_rate'] < 95:
            recommendations.append("1. Focus on improving tools with lowest success rates")
            
            # Find most problematic tools
            for tool_name, stats in sorted_tools:
                if stats['success_rate'] < 80:
                    recommendations.append(f"   - {tool_name}: Needs major improvements ({stats['success_rate']:.1f}% success)")
        ...
        if self.results['failure_patterns'].get('import_failed', 0) > 5:
            ...
            recommendations.append("   - Add support for non-standard MusicXML elements")
        
        if any(stats['success_rate'] < 90 for _, stats in self.results['tool_results'].items()):
            ...
            recommendations.append("   - Add graceful degradation for unsupported features")
    ...
    def save_results(self, output_dir: Path = Path("validation_results")):
        ...
        with open(output_dir / f"compatibility_matrix_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt", 'w') as f:
            f.write(matrix_report)
        
        # Generate and save detailed error report
        error_report = self.generate_error_report()
        with open(output_dir / f"error_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt", 'w') as f:
