...
class SimpleStressTest:
    ...
    async def simulate_client_load(self, duration_seconds: int, concurrent_clients: int):
        """Simulate multiple concurrent clients"""
        logger.info(f"Starting {concurrent_clients} concurrent clients for {duration_seconds}s")
    ...
    async def run_test(self, hours: float, concurrent_users: int, enable_chaos: bool = True):
        """Run the complete stress test"""
        duration_seconds = int(hours * 3600)
        
        logger.info(f"Starting stress test for {hours} hours with {concurrent_users} users")
...
async def main():
    import argparse
    parser = argparse.ArgumentParser(description="Simple stress test for music21 MCP server")
    parser.add_argument("--hours", type=float, default=0.1, help="Test duration in hours")
