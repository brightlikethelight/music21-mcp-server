        ...
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
                ...
                return {
                    ...
                    "active_requests": health_data.get("metrics", {}).get(
                        "active_requests", 0
                    ),
...
class TerminalDashboard:
    ...
    def draw_server_metrics(self, metrics: Dict[str, Any], start_row: int):
        ...
        # Status
        status_color = {"healthy": 1, "degraded": 2, "unhealthy": 3}.get(
            server["status"], 3
        )
        ...
        success_color = (
            ...
            else 2 if server["success_rate"] >= 90 else 3
        )
        ...
        # Memory
        mem_color = (
            1 if server["memory_mb"] < 2000 else 2 if server["memory_mb"] < 3000 else 3
        )
        ...
        open_breakers = [
            name
            for name, b in server["circuit_breakers"].items()
            if b["state"] == "open"
        ]
        if open_breakers:
            self.stdscr.attron(curses.color_pair(3))
            self.stdscr.addstr(
                row, 2, f"Open Circuit Breakers: {', '.join(open_breakers)}"
            )
            self.stdscr.attroff(curses.color_pair(3))
            row += 1
    ...
    def draw_system_metrics(self, metrics: Dict[str, Any], start_row: int):
        ...
        # CPU
        cpu_color = (
            1 if system["cpu_percent"] < 70 else 2 if system["cpu_percent"] < 90 else 3
        )
        ...
        mem_color = (
            ...
            else 2 if system["memory_percent"] < 90 else 3
        )
        self.stdscr.addstr(row, 2, "Memory: ")
        self.stdscr.attron(curses.color_pair(mem_color))
        ...
        self.stdscr.addstr(
            row,
            2,
            f"Disk: {system['disk_percent']:.1f}% ({system['disk_free_gb']:.1f}GB free)",
        )
        row += 1
        ...
        self.stdscr.addstr(
            row,
            2,
            f"Network - Sent: {system['network_sent_mb']:.1f}MB, Recv: {system['network_recv_mb']:.1f}MB",
        )
        row += 1
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Monitor Music21 MCP Server stress test"
    )
