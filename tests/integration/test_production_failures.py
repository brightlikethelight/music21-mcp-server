...
try:
    from music21 import corpus, stream, note, chord, key, metadata
...
class TestProductionFailures:
    ...
    def test_tools_visibility_in_claude_desktop(self):
        ...
        expected_tools = [
            "import_score", "list_scores", "score_info", "export_score",
            "delete_score", "key_analysis", "chord_analysis", "harmony_analysis",
            "voice_leading_analysis", "pattern_recognition", "harmonization",
            "counterpoint", "style_imitation", "health_check", "cleanup_memory"
        ]
        ...
        if hasattr(server, 'import_score'):
            func = getattr(server, 'import_score')
    ...
    async def test_timeout_scenarios(self):
        ...
        with pytest.raises(asyncio.TimeoutError):
            await asyncio.wait_for(
                slow_tool_func(test_param="value"),
                timeout=2.0
            )
        ...
        with patch('urllib.request.urlopen') as mock_urlopen:
            ...
            if "error" in result:
                assert "timeout" in result["error"].lower() or "error" in result["status"]
    ...
    async def test_concurrent_requests(self):
        ...
        tasks = [
            analyze(), get_info(), export(), modify(),
            analyze(), get_info(), export(), modify(),
        ]
        ...
        failures = sum(1 for r in results if isinstance(r, Exception) or (isinstance(r, dict) and r.get("status") == "error"))
        ...
        stress_successes = sum(1 for r in stress_results 
    ...
    async def test_docker_cloud_environment(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                if result["status"] == "error":
                    print(f"Expected permission error: {result.get('error', 'Unknown')}")
        ...
        for i in range(10):
            try:
                ...
                if current_memory > memory_limit_mb:
                    print(f"⚠️  Memory limit exceeded: {current_memory:.1f}MB > {memory_limit_mb:.1f}MB")
        ...
        print(f"Completed {large_operations} operations with {memory_errors} memory issues")
    ...
    async def test_edge_cases_and_corner_cases(self):
        ...
        for weird_id in weird_ids:
            try:
                ...
                assert "status" in result
                if result["status"] == "error":
                    print(f"ID '{weird_id[:20]}...' rejected: {result.get('error', 'Unknown')}")
            except Exception as e:
        ...
        malformed_tests = [
            ...
            {"score_id": {"dict": "value"}, "source": "bach/bwv66.6", "source_type": "corpus"},
            ...
            {"score_id": "test", "source": "bach/bwv66.6", "source_type": "corpus", 
    ...
    async def test_memory_leaks(self):
        ...
        for cycle in range(10):
            ...
            if current_memory > initial_memory * 1.5:  # 50% increase
                leak_detected = True
                print(f"⚠️  Potential memory leak: {initial_memory:.1f}MB -> {current_memory:.1f}MB")
        
        # Analyze memory trend
        memory_increase = memory_readings[-1] - memory_readings[0]
        avg_increase_per_cycle = memory_increase / len(memory_readings)
        
        print(f"Memory trend: Start={memory_readings[0]:.1f}MB, End={memory_readings[-1]:.1f}MB")
