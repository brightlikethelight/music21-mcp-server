...
class TestFastMCPServer:
    ...
    def test_rate_limiting_functionality(self):
        ...
        assert test_limiter.acquire() == True
        assert test_limiter.acquire() == True
        assert test_limiter.acquire() == True
        
        # Should be rate limited now
        assert test_limiter.acquire() == False
    ...
    async def test_rate_limiting_with_tools(self):
        ...
        try:
            ...
            assert "error" not in result1 or "Rate limit" not in result1.get("error", "")
            
            result2 = await import_score("test2", "bach/bwv66.6", "corpus") 
            assert "error" not in result2 or "Rate limit" not in result2.get("error", "")
    ...
    async def test_tool_interconnection_workflow(self):
        ...
        # Test MIDI export
        midi_result = await export_tool.execute(
            score_id="workflow_test",
            format="midi"
        )
    ...
    async def test_concurrent_access_patterns(self):
        ...
        successful_imports = [r for r in results if isinstance(r, dict) and r.get("status") == "success" and 
                             (r.get("title") or r.get("score_id") or "import" in str(r))]
        ...
        concurrent_scores = [s for s in final_list["scores"] if s["id"].startswith("concurrent_")]
    ...
    async def test_resource_endpoints(self):
        ...
        from music21_mcp.server import get_scores_list, get_score_metadata
