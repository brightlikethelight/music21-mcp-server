    ...
    async def test_import_score_tool(self) -> bool:
        ...
        try:
            ...
            if result.get("status") == "success":
                ...
                self.log_error("import_corpus", f"Failed to import corpus: {result.get('error')}", "high")
                return False
            ...
            if result.get("status") == "error":
                ...
                self.log_error("import_error", "Failed to handle invalid import", "medium")
...
C E G c |]"""
            ...
            if result.get("status") == "success":
                ...
                self.log_error("import_custom", f"Failed to create custom score: {result.get('error')}", "medium")
    ...
    async def test_key_analysis_tool(self) -> bool:
        ...
        try:
            ...
            if result.get("status") == "success":
                ...
                if confidence < 0.1:
                    self.log_error("key_confidence", f"Very low confidence: {confidence}", "medium")
            else:
                self.log_error("key_analysis", f"Key analysis failed: {result.get('error')}", "high")
                return False
            ...
            if result.get("status") == "success":
                ...
                if len(set(keys)) > 3:  # Too much disagreement
                    self.log_error("key_consensus", "Algorithms strongly disagree on key", "medium")
            else:
                self.log_error("key_all", f"Consensus analysis failed: {result.get('error')}", "medium")
            
            return len([e for e in self.errors if e["severity"] == "high"]) == 0
            
        except Exception as e:
            self.log_error("key_tool", f"Key analysis tool crashed: {str(e)}", "critical")
            return False
        ...
        try:
            ...
            if result.get("status") == "success":
                ...
                if len(roman_numerals) < 4:
                    self.log_error("harmony_detection", f"Only found {len(roman_numerals)} chords in I-IV-V-I", "medium")
                ...
                self.log_error("harmony_analysis", f"Harmony analysis failed: {result.get('error')}", "high")
                return False
            
            return len([e for e in self.errors if e["severity"] == "high"]) == 0
            
        except Exception as e:
            self.log_error("harmony_tool", f"Harmony analysis tool crashed: {str(e)}", "critical")
            return False
        ...
        try:
            ...
            if result.get("status") == "success":
                ...
                if len(melodic) == 0 and len(rhythmic) == 0:
                    self.log_error("pattern_detection", "No patterns found in obvious sequence", "high")
            else:
                self.log_error("pattern_analysis", f"Pattern analysis failed: {result.get('error')}", "high")
                return False
            
            return len([e for e in self.errors if e["severity"] == "high"]) == 0
            
        except Exception as e:
            self.log_error("pattern_tool", f"Pattern recognition tool crashed: {str(e)}", "critical")
            return False
    ...
    async def test_tool_integration_workflow(self) -> bool:
        ...
        try:
            ...
            if key_result.get("status") != "success":
                self.log_error("workflow_key", "Workflow failed at key analysis", "high")
                return False
            ...
            if not any(s["id"] == workflow_score_id for s in scores):
                self.log_error("workflow_persistence", "Score not found in list", "high")
                return False
