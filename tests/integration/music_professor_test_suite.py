...
try:
    from music21 import analysis, chord, converter, corpus, harmony, key, meter, pitch
...
class MusicProfessorTester:
        ...
        try:
            ...
            for piece_path, expected_key in test_pieces:
                try:
                    ...
                    analysis_result = (
                        await self.harmonic_analyzer.analyze_key_signature(score)
                    )
        ...
        try:
            ...
            for progression in test_progressions:
                try:
                    ...
                    from music21 import duration, meter, stream
                    ...
                    start_time = time.time()
                    harmony_result = (
                        await self.harmonic_analyzer.analyze_harmonic_progression(score)
                    )
                    analysis_time = time.time() - start_time
                    ...
                    if "chords" in harmony_result:
                        ...
                        print(
                            f"     Found {chords_found} chords (expected {expected_chords})"
                        )
            ...
            print(
                f"✅ Harmonic progression test completed - {progression_issues} issues found"
            )
            ...
            for melody_test in melody_tests:
                try:
                    ...
                    if "intervals" in melodic_result:
                        ...
                        print(
                            f"     Found {intervals_found} intervals (expected {expected_intervals})"
                        )
                    ...
                    if "patterns" in melodic_result:
                        ...
                        if (
                            len(patterns) == 0
                            and melody_test["expected_pattern"] != "none"
                        ):
                            self.log_musical_error(
                                melody_test["name"],
                                "pattern_recognition",
        ...
        try:
            ...
            for rhythm_test in rhythm_tests:
                try:
                    ...
                    start_time = time.time()
                    rhythm_result = (
                        await self.rhythm_analyzer.analyze_rhythmic_patterns(
                            rhythm_stream
                        )
                    )
                    analysis_time = time.time() - start_time
    ...
    async def test_non_western_music_handling(self) -> bool:
        ...
        try:
                ...
                try:
                    ...
                    try:
                        ...
                        if key_result and "key" in key_result:
                            ...
                            print(
                                f"     Key analysis: {detected_key} (confidence: {confidence:.2f})"
                            )

                            # Should at least indicate uncertainty for truly non-Western scales
                            if (
                                test["culture"] in ["Arabic", "Indian"]
                                and confidence > 0.8
                            ):
                                print(
                                    f"     Note: High confidence on non-Western scale may indicate over-confidence"
                                )
            ...
            print(
                f"✅ Non-Western music test completed - {non_western_issues} issues found"
            )
