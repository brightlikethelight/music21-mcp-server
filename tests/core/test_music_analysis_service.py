...
class TestMusicAnalysisServiceCore:
    ...
    async def test_import_invalid_corpus_piece(self, service):
        ...
        assert "not found" in result["error"].lower() or "could not" in result["error"].lower()
    ...
    async def test_analyze_harmony_invalid_type(self, service):
        ...
        assert result["status"] == "error"
        assert "analysis_type" in result["error"].lower() or "invalid" in result["error"].lower()
    ...
    async def test_recognize_invalid_pattern_type(self, service):
        ...
        assert result["status"] == "error"
        assert "pattern_type" in result["error"].lower() or "invalid" in result["error"].lower()
    ...
    def test_get_available_tools(self, service):
        ...
        expected_tools = [
            "import_score", "list_scores", "get_score_info", "delete_score",
            "analyze_key", "analyze_harmony", "analyze_voice_leading", 
            "recognize_patterns"
        ]
    ...
    async def test_empty_score_id(self, service):
        ...
        assert result["status"] == "error"
        assert "score_id" in result["error"].lower() or "empty" in result["error"].lower()
