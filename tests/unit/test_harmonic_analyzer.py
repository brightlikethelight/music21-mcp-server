...
from src.music21_mcp.core.harmonic_analyzer import (ChordSubstitutionType,
...
class TestHarmonicAnalyzer:
    ...
    async def test_voice_leading_analysis(self, analyzer, four_part_chorale):
        ...
        assert (
            result.voice_ranges["voice_1"]["average"]
            > result.voice_ranges["voice_2"]["average"]
        )
    ...
    async def test_parallel_motion_detection(self, analyzer):
        ...
        assert any(
            err["type"] == VoiceLeadingError.PARALLEL_FIFTHS.value
...
class TestVoiceLeadingRules:
    ...
    async def test_large_leap_detection(self, analyzer):
        ...
        if result.errors:
            assert any(
                err["type"] == VoiceLeadingError.LARGE_LEAP.value
                for err in result.errors
            )
