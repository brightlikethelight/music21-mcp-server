...
class TestRateLimiter:
    ...
    async def test_check_rate_limit_endpoint_specific(self, rate_limiter):
        ...
        allowed, metadata = await rate_limiter.check_rate_limit(identifier, endpoint=endpoint)
        assert allowed is True
    ...
    async def test_cleanup_empty_histories(self, rate_limiter):
        ...
        # Add empty history
        rate_limiter.request_history["empty_user"] = rate_limiter.request_history.default_factory()
    ...
    async def test_cleanup_loop_exception_handling(self, rate_limiter):
        ...
        with patch.object(rate_limiter, "cleanup_expired", side_effect=Exception("cleanup error")):
