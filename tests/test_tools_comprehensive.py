...
# Import tools
from music21_mcp.tools import (ChordAnalysisTool, DeleteScoreTool,
                               ExportScoreTool, ImportScoreTool,
                               KeyAnalysisTool, ListScoresTool, ScoreInfoTool)
...
class TestListScoresTool:
    ...
    @pytest.mark.asyncio
    async def test_list_multiple_scores(
        self, score_manager, simple_score, complex_score
    ):
...
class TestEdgeCasesAndPerformance:
    ...
    async def test_concurrent_operations(self, score_manager):
        ...
        tasks = [import_tool.execute(f"score_{i}", "C4 D4 E4 F4 G4") for i in range(10)]
