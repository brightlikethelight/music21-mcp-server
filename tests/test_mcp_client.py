...
class MCPClientTester:
    ...
    def __init__(self):
        self.server_path = (
            Path(__file__).parent.parent / "src" / "music21_mcp" / "server.py"
        )
    ...
    async def test_stdio_connection(self):
        ...
        async with stdio_client([sys.executable, "-m", "music21_mcp.server"]) as (
            read_stream,
            write_stream,
        ):
            async with ClientSession(read_stream, write_stream) as session:
                ...
                analysis = await session.call_tool(
                    "analyze_key", {"score_id": "test_melody"}
                )
...
async def test_real_world_scenarios():
    ...
    for scenario in scenarios:
        ...
        for tool_name, args in scenario["tools"]:
            print(
                f"      - {tool_name}({json.dumps(args, indent=0).replace(chr(10), '')})"
            )
