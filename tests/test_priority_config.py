...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", f"{CORE_PRIORITY}: Core music21 tests (95% effort) - these MUST pass"
    )
    config.addinivalue_line(
        "markers",
...
def pytest_collection_modifyitems(config, items):
    ...
    print(
        f"🎯 Target: 95% core, 5% adapter (actual: {core_pct:.1f}% core, {adapter_pct:.1f}% adapter)"
    )
...
def run_core_tests_only():
    ...
    pytest.main(
        ["tests/core/", "-v", "--tb=short", f"-m {CORE_PRIORITY}", "--no-header"]
    )
