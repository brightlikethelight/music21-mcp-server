...
class RhythmAnalyzer:
    ...
    async def _analyze_tempo(self, score: stream.Score) -> TempoAnalysis:
        ...
        if tempos:
            ...
            tempo_stability = (
                1.0 - (tempo_variance / (average_tempo**2))
                if average_tempo > 0
                else 1.0
            )
            ...
            rubato_likelihood = await self._detect_rubato_likelihood(
                score, tempo_stability
            )
    ...
    async def _detect_tempo_from_durations(
    ...
    async def _detect_rubato_likelihood(
        self, score: stream.Score, tempo_stability: float
    ) -> float:
        ...
        for element in score.flatten():
            if isinstance(element, expressions.Expression):
                total_marks += 1
                if any(
                    term in str(element).lower()
                    for term in ["rubato", "espressivo", "freely", "ad lib", "piacere"]
                ):
                    expressive_marks += 1
    ...
    async def _analyze_meter(self, score: stream.Score) -> MeterAnalysis:
                ...
                time_sigs.append(
                    {
                        ...
                        "is_compound": inferred.numerator % 3 == 0
                        and inferred.numerator > 3,
                    }
                )
            ...
            meter_changes.append(
                {
                    ...
                    "type": self._classify_meter_change(
        ...
        is_mixed_meter = (
            len(set(f"{ts['numerator']}/{ts['denominator']}" for ts in time_sigs)) > 1
        )
        ...
        if len(time_sigs) <= 1:
            ...
            changes_per_measure = len(meter_changes) / len(
                score.getElementsByClass(stream.Measure)
            )
    ...
    def _classify_meter_change(
        self, prev: meter.TimeSignature, curr: meter.TimeSignature
    ) -> str:
    ...
    def _analyze_beat_hierarchy(self, ts: meter.TimeSignature) -> Dict[str, Any]:
        ...
        hierarchy = {"primary_beat": 1.0, "beat_pattern": [], "accent_pattern": []}
        ...
        if beat_pattern:
            hierarchy["beat_pattern"] = [
                {
                    "beat": i + 1,
                    "duration": beat.duration.quarterLength,
                    "weight": beat.weight,
                }
    ...
    async def _detect_hypermeter(
        self, score: stream.Score, primary_meter: meter.TimeSignature
        ...
        for i, measure in enumerate(measures):
            ...
            for element in measure:
                if isinstance(element, expressions.Fermata):
                    ...
                    isinstance(element, note.Rest)
                    and element.duration.quarterLength
                    >= measure.barDuration.quarterLength * 0.5
                ):
                    has_phrase_end = True
                    break
    ...
    async def _extract_rhythmic_patterns(
        self, score: stream.Score, min_length: int, min_occurrences: int
        ...
        for part in score.parts:
            ...
            for element in part.flatten():
                if isinstance(element, (note.Note, note.Rest, chord.Chord)):
                    ...
                    current_measure_rhythms.append(
    ...
    def _find_rhythm_patterns(
        self, rhythm_sequence: List[List[float]], min_length: int, min_occurrences: int
    ) -> List[RhythmicPattern]:
                ...
                for i in range(len(flat_sequence) - length + 1):
                    if self._rhythms_match(flat_sequence[i : i + length], pattern):
                        ...
                        measure_num = (
                            next(
                                j
                                for j, ms in enumerate(measure_starts[:-1])
    ...
    def _rhythms_match(
        self, seq1: List[float], seq2: List[float], tolerance: float = 0.01
    ) -> bool:
    ...
    def _classify_rhythm_pattern(self, pattern: List[float]) -> str:
        ...
        for name, known_pattern in self.PATTERN_LIBRARY.items():
            if self._rhythms_match(
                pattern[: len(known_pattern)], known_pattern, tolerance=0.1
            ):
                return name
    ...
    def _deduplicate_patterns(
        self, patterns: List[RhythmicPattern]
    ) -> List[RhythmicPattern]:
    ...
    async def _calculate_rhythmic_complexity(
        self, score: stream.Score
    ) -> RhythmicComplexity:
        ...
        if durations:
            ...
            normalized_entropy = (
                rhythm_entropy / np.log(len(duration_counts))
        ...
        for n in score.flatten().notes:
            if isinstance(n, (note.Note, chord.Chord)):
                ...
                if beat_strength is not None and beat_strength < 0.5:
                    ...
                    if any(
                        isinstance(a, articulations.Accent) for a in n.articulations
                    ):
                        syncopated_notes += 1
    ...
    async def _analyze_syncopation(
        self, score: stream.Score, meter_analysis: MeterAnalysis
    ) -> float:
            ...
            for n in part.flatten().notes:
                if isinstance(n, (note.Note, chord.Chord)):
                    ...
                    if (
                        ...
                        and n.duration.quarterLength > 1.0
                    ):
                        syncopation_events.append(
                            {
                                "type": "long_weak_beat",
    ...
    async def _analyze_groove(self, score: stream.Score) -> Dict[str, Any]:
        ...
        groove_analysis["groove_consistency"] = (
            await self._calculate_groove_consistency(score)
        )
        ...
        for part in score.parts:
            ...
            for i in range(len(notes) - 1):
                ...
                if (
                    ...
                    and n2.duration.quarterLength == 0.5
                    and n2.offset == n1.offset + 0.5
                ):
    ...
    async def _calculate_groove_consistency(self, score: stream.Score) -> float:
        ...
        regularity = sum(count**2 for count in position_counts.values()) / (
            total_notes**2
        )
    ...
    async def _calculate_rhythmic_drive(self, score: stream.Score) -> float:
        ...
        rest_factor = (
            1.0 - (len(long_rests) / (note_count + len(rests)))
            if (note_count + len(rests)) > 0
            else 1.0
        )
    ...
    async def _analyze_beat_strength(
        ...
        sample_indices = np.linspace(
            0, len(measures) - 1, min(10, len(measures)), dtype=int
        )

        for idx in sample_indices:
            measure = measures[idx]
            measure_profile = {"measure": idx + 1, "beats": []}
            ...
            for beat_num in range(1, int(meter_analysis.primary_meter.numerator) + 1):
                ...
                if beat_notes:
                    avg_velocity = np.mean(
                        [n.volume.velocity for n in beat_notes if n.volume.velocity]
                    )
                    has_accent = any(
                    ...
                    strength = (
                        ...
                        if beat_num == 1
                            ...
                            BeatStrength.MEDIUM
                            if beat_num
                            == (meter_analysis.primary_meter.numerator + 1) // 2
    ...
    async def analyze_tempo_stability(
        ...
        if len(onset_times) < 10:
            return {"stability": 1.0, "variance": 0.0, "trend": "stable"}
