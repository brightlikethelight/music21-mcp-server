...
class HarmonicAnalyzer:
                ...
                if i > 0:
                    ...
                    if self._is_tonic_prolongation(prev_rn, rn):
                        result.tonic_prolongations.append({
                            'start_measure': self._get_measure_number(chords[i-1], measures),
                    ...
                    if self._is_predominant(rn):
                        result.predominant_chords.append({
                            ...
                            'leads_to': self._get_next_function(i, chords, score_key)
                        })
            ...
            if include_tensions:
                ...
                if tensions:
                    result.tension_analysis.append({
                        ...
                        'chord': symbol,
                        'tensions': tensions,
                        'tension_resolution': self._get_tension_resolution(ch, i, chords)
                    })
        
        # Detect modal interchange
        result.modal_interchanges = await self._detect_modal_interchange(
            score, chords
        )
            ...
            for start in range(len(roman_progression) - length):
                ...
                for i in range(len(roman_progression) - length):
                    if roman_progression[i:i + length] == pattern:
                        occurrences.append({
                            'start_index': i,
                            'start_measure': self._get_chord_measure(chords[i], score)
                        })
                ...
                if len(occurrences) >= min_occurrences:
                    ...
                    for i in range(1, len(occurrences)):
                        ...
                        if root1 and root2:
                            intervals.append(
                                interval.Interval(root1, root2).semitones
                            )
                    
                    sequences.append(HarmonicSequence(
                        pattern=pattern,
                        intervals=intervals,
                        occurrences=occurrences,
                        sequence_type=seq_type,
                        direction='descending' if all(i < 0 for i in intervals) 
                                 else 'ascending' if all(i > 0 for i in intervals)
                                 else 'mixed',
                        real_or_tonal='real' if len(set(intervals)) == 1 else 'tonal'
                    ))
    ...
    def _is_tonic_prolongation(
        self,
        prev: roman.RomanNumeral,
        curr: roman.RomanNumeral
    ) -> bool:
        """Check if progression prolongs tonic"""
        tonic_chords = ['I', 'i', 'I6', 'i6', 'I64', 'i64']
        prolonging_chords = ['V', 'V6', 'V7', 'vii°', 'vii°6', 'IV', 'iv']
        
        return (prev.figure in tonic_chords and 
                curr.figure in prolonging_chords) or \
               (prev.figure in prolonging_chords and 
                curr.figure in tonic_chords)
    ...
    def _is_predominant(self, rn: roman.RomanNumeral) -> bool:
        ...
        predominants = ['ii', 'II', 'ii6', 'II6', 'ii7', 'II7',
                       'iv', 'IV', 'iv6', 'IV6', 'N6', 'It+6',
                       'Fr+6', 'Ger+6']
    ...
    def _get_prep_strength(
        self,
        prep: roman.RomanNumeral,
        dom: roman.RomanNumeral
    ) -> float:
    ...
    def _identify_cadence_type(
        self,
        penult: roman.RomanNumeral,
        final: roman.RomanNumeral
    ) -> Optional[str]:
        ...
        # Plagal
        elif ('IV' in penult.figure or 'iv' in penult.figure) and final.scaleDegree == 1:
            return 'plagal'
    ...
    def _determine_phrase_model(self, cadences: List[Dict]) -> str:
        ...
        if (len(cadences) >= 2 and 
            cadences[0]['type'] == 'half' and 
            'authentic' in cadences[1]['type']):
            return 'period'
    ...
    def _calculate_tonal_strength(self, analysis: FunctionalHarmonyAnalysis) -> float:
        ...
        for i in range(len(analysis.functions) - 1):
            if (analysis.functions[i] == HarmonicFunction.PREDOMINANT and
                analysis.functions[i+1] == HarmonicFunction.DOMINANT):
                score += 0.3
            elif (analysis.functions[i] == HarmonicFunction.DOMINANT and
                  analysis.functions[i+1] == HarmonicFunction.TONIC):
                score += 0.5
    ...
    def _align_voices(
        self,
        voice1: List[note.Note],
        voice2: List[note.Note]
    ...
    def _check_voice_leading_errors(
        ...
        if vl.parallelOctave():
            errors.append({
                'type': VoiceLeadingError.PARALLEL_OCTAVES,
                'severity': 'high'
            })
        ...
        if strict:
            if vl.hiddenFifth():
                errors.append({
                    'type': VoiceLeadingError.DIRECT_FIFTHS,
                    'severity': 'medium'
                })
            
            if vl.hiddenOctave():
                errors.append({
                    'type': VoiceLeadingError.DIRECT_OCTAVES,
                    'severity': 'medium'
                })
        ...
        for i in range(len(parts)):
            for j in range(i + 1, len(parts)):
                ...
                if rhythm1 and rhythm2:
                    ...
                    same_rhythm = sum(1 for k in range(min_len) 
    ...
    async def _detect_modal_interchange(
        ...
        for i, ch in enumerate(chords):
            ...
            try:
                ...
                if (rn_parallel.pitches == ch.pitches and 
                    rn_current.pitches != ch.pitches):
                    interchanges.append({
                        'measure': self._get_chord_measure(ch, score),
                        'chord': self._get_jazz_chord_symbol(ch),
                        'borrowed_from': str(parallel_key),
                        'function_in_parallel': rn_parallel.figure,
                        'modal_color': 'darker' if self.current_key.mode == 'major' 
                                      else 'brighter'
                    })
            except:
                pass
    ...
    def _filter_sequences(
        ...
        sequences.sort(
            key=lambda s: (len(s.occurrences), len(s.pattern)),
            reverse=True
        )
    ...
    def _consolidate_key_regions(
        ...
        for region in regions[1:]:
            if (region['key'] == consolidated[-1]['key'] and
                region['start_measure'] <= consolidated[-1]['end_measure'] + 1):
