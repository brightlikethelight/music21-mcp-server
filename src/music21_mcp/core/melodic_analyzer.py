...
class MelodicAnalyzer:
    ...
    async def analyze_melodic_contour(
        ...
        result.complexity_score = self._calculate_contour_complexity(result.contour_vector)
    ...
    async def analyze_cross_cultural_elements(
        ...
        result.cultural_markers = self._extract_cultural_markers(
            notes, intervals, result.detected_styles[0][0] if result.detected_styles else None
        )
        ...
        for i in range(n_melodies):
            ...
            if len(similar_indices) > 0:
                result.similar_phrases.append({
                    ...
                    'similarity_scores': result.similarity_matrix[i, similar_indices].tolist()
                })
        ...
        for i in range(0, len(measures), section_size):
            ...
            if notes:
                pitch_range = {
                    ...
                    'range': max(n.pitch.midi for n in notes) - min(n.pitch.midi for n in notes)
                }
        ...
        if all_notes:
            ...
            for idx in climax_indices:
                climax = {
                    ...
                    'dynamic': all_notes[idx].volume.velocity if track_dynamics else None,
    ...
    def _classify_contour(self, contour_vector: List[int]) -> ContourType:
        ...
        if (sum(first_half) > len(first_half) * 0.3 and 
            sum(second_half) < -len(second_half) * 0.3):
            return ContourType.ARCH
        elif (sum(first_half) < -len(first_half) * 0.3 and 
              sum(second_half) > len(second_half) * 0.3):
    ...
    def _create_motive(
        ...
        if notes[0].duration.quarterLength > 0:
            for n in notes:
                rhythm.append(n.duration.quarterLength / notes[0].duration.quarterLength)
        else:
        ...
        # Get measure numbers
        start_measure = notes[0].measureNumber if hasattr(notes[0], 'measureNumber') else 0
        end_measure = notes[-1].measureNumber if hasattr(notes[-1], 'measureNumber') else 0
        ...
        for i in range(start_from, len(all_notes) - motive_length + 1):
            ...
            for transformation in PatternTransformation:
                ...
                if similarity >= self.similarity_threshold:
                    occurrences.append({
                        'start_index': i,
                        'measure': candidate_notes[0].measureNumber 
                                  if hasattr(candidate_notes[0], 'measureNumber') else 0,
    ...
    def _compare_interval_sequences(
        self,
        seq1: List[int],
        seq2: List[int]
    ) -> float:
            ...
            for section_start in range(0, total_measures, section_size):
                ...
                if section_occurrences:
                    ...
                    development.append({
                        ...
                        'development_intensity': len(section_occurrences) / section_size
                    })
    ...
    def _calculate_motivic_coherence(self, motives: List[MelodicMotive]) -> float:
        ...
        if len(motives) > 1:
            ...
            relationship_score = relationships / possible_relationships if possible_relationships > 0 else 0
    ...
    def _are_motives_related(
        self,
        motive1: MelodicMotive,
        motive2: MelodicMotive
    ) -> bool:
    ...
    def _compare_patterns(
        ...
        if transformation == PatternTransformation.EXACT:
            ...
            if all(n1.pitch.midi == n2.pitch.midi 
            ...
            if pattern1_intervals == pattern2_intervals:
                transposition = pattern2_notes[0].pitch.midi - pattern1_notes[0].pitch.midi
    ...
    def _detect_ornamentations(self, notes: List[note.Note]) -> List[Dict[str, Any]]:
        ...
        for i in range(1, len(notes) - 1):
            ...
            if notes[i].duration.quarterLength < 0.25:
                ...
                if (notes[i-1].pitch == notes[i+1].pitch and
                    abs(notes[i].pitch.midi - notes[i-1].pitch.midi) <= 2):
                    ornament_type = 'mordent' if notes[i].pitch > notes[i-1].pitch else 'lower_mordent'
        ...
        for i in range(len(notes) - 3):
            if (notes[i].pitch == notes[i+2].pitch and
                notes[i+1].pitch == notes[i+3].pitch and
                abs(notes[i].pitch.midi - notes[i+1].pitch.midi) <= 2):
                ornamentations.append({
                    'index': i,
                    'type': 'trill',
                    'pitches': [notes[i].nameWithOctave, notes[i+1].nameWithOctave],
                    'duration': sum(n.duration.quarterLength for n in notes[i:i+4])
                })
        ...
        for i, note in enumerate(notes):
            ...
            if hasattr(note.pitch, 'microtone') and note.pitch.microtone:
                microtones.append({
                    ...
                    'direction': 'sharp' if note.pitch.microtone.cents > 0 else 'flat'
                })
    ...
    def _extract_cultural_markers(
        ...
        markers['characteristic_intervals'] = [
            interval for interval, count in interval_counts.most_common(5)
            if count >= 2
        ]
    ...
    def _calculate_pairwise_similarity(
        ...
        if method == 'interval':
            ...
            interval_sim = self._calculate_pairwise_similarity(melody1, melody2, 'interval')
            contour_sim = self._calculate_pairwise_similarity(melody1, melody2, 'contour')
        ...
        for theme_idx in potential_themes:
            ...
            if len(variation_indices) > 0:
                variations.append({
                    ...
                    'similarity_scores': similarity_matrix[theme_idx, variation_indices].tolist(),
