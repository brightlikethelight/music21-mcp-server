...
class AdvancedTheoryAnalyzer:
    ...
    async def analyze_scale_degrees(
        self, score: stream.Score, key_context: Optional[key.Key] = None
    ) -> ScaleDegreeAnalysis:
        """
        ...
        return ScaleDegreeAnalysis(
            ...
            chromatic_percentage=(
                chromatic_count / len(melody_notes) if melody_notes else 0
            ),
    ...
    async def calculate_interval_vector(
        ...
        consonance_ratio = (
            consonant_intervals / total_intervals if total_intervals > 0 else 0
        )
                ...
                if isinstance(n, note.Note):
                    if n.pitch.pitchClass not in diatonic_pcs:
                        ...
                        if i > 0 and i < len(notes) - 1:
                            ...
                            if isinstance(prev_note, note.Note) and isinstance(
                                next_note, note.Note
                            ):
                                voice_leading_events.append(
                                    {
                                        "chromatic_pitch": n.pitch.name,
                                        "approach": interval.Interval(
                                            prev_note.pitch, n.pitch
                                        ).niceName,
                                        "resolution": interval.Interval(
                                            n.pitch, next_note.pitch
                                        ).niceName,
                                        "type": self._classify_voice_leading(
                                            prev_note, n, next_note
                                        ),
                                    }
                                )
    ...
    async def _classify_chromatic_function(
        ...
        # Calculate intervals
        approach_interval = interval.Interval(
            prev_note.pitch, chromatic_note.pitch
        ).semitones
        departure_interval = interval.Interval(
            chromatic_note.pitch, next_note.pitch
        ).semitones
        ...
        if chromatic_note.pitch.pitchClass in [
            p.pitchClass for p in parallel_key.pitches
        ]:
    ...
    def _classify_voice_leading(
        self, prev_note: note.Note, chromatic_note: note.Note, next_note: note.Note
    ) -> str:
    ...
    async def _find_modal_mixture(
        self, score: stream.Score, key_context: key.Key
    ) -> List[Dict[str, Any]]:
            ...
            if c.isMajorTriad() or c.isDominantSeventh():
                ...
                if i < len(chords) - 1:
                    ...
                    if next_chord.root():
                        ...
                        if (
                            resolution_interval.semitones == -7
                            or resolution_interval.semitones == 5
                        ):
    ...
    async def _detect_augmented_sixths(
        ...
        for c in score.flatten().getElementsByClass(chord.Chord):
            ...
            has_aug_sixth = any(
                intv.name == "A6"
                or (intv.semitones == 10 and intv.diatonic.generic.value == 6)
                for intv in intervals
            )
    ...
    def _classify_augmented_sixth(
        self, chord: chord.Chord, key_context: key.Key
    ) -> Optional[str]:
        ...
        if chord_size == 3:
            ...
            has_perfect_fifth = any(intv.name == "P5" for intv in chord.intervalVector)
                    ...
                    borrowed.append(
                        {
                            ...
                            "borrowed_from": (
                                "minor" if key_context.mode == "major" else "major"
                            ),
                        }
                    )
            except:
    ...
    async def analyze_phrase_structure(
        self, score: stream.Score, include_motivic_analysis: bool = True
    ) -> PhraseStructure:
        """
                ...
                phrases.append(
                    {
                        ...
                        "ending_type": (
                            "fermata"
                            if has_fermata
                            else "rest" if has_long_rest else "cadence"
                        ),
                    }
                )
    ...
    async def _analyze_cadences(
        self, score: stream.Score, phrases: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        ...
        for phrase in phrases:
            ...
            if end_measure_idx < len(measures) and end_measure_idx > 0:
                ...
                if pen_harmony and final_harmony:
                    cadence_type = self._classify_cadence_type(
                        pen_harmony, final_harmony
                    )
                    ...
                    cadences.append(
                        {
                            ...
                            "type": cadence_type,
                            "chords": [
                                pen_harmony.pitchesChr,
                                final_harmony.pitchesChr,
                            ],
                            "strength": self._calculate_cadence_strength(
                                cadence_type, final
                            ),
                        }
                    )
    ...
    def _classify_cadence_type(self, chord1: chord.Chord, chord2: chord.Chord) -> str:
        ...
        if chord2.isDominantSeventh() or (
            chord2.isMajorTriad() and chord2.root().name in ["G", "D", "A", "E", "B"]
        ):
            return "half"
    ...
    def _calculate_cadence_strength(
        self, cadence_type: str, final_measure: stream.Measure
    ) -> float:
    ...
    def _find_elisions(
        self, phrases: List[Dict[str, Any]], cadences: List[Dict[str, Any]]
    ) -> List[int]:
    ...
    def _analyze_phrase_rhythm(self, phrases: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_length": (
                sum(p["length"] for p in phrases) / len(phrases) if phrases else 0
            ),
    ...
    async def _analyze_motives(
        ...
        motives = {"melodic_motives": [], "rhythmic_motives": [], "transformations": []}
    ...
    async def analyze_dissonance_treatment(self, score: stream.Score) -> Dict[str, Any]:
        """
                                ...
                                dissonances.append(
                                    {
                                        ...
                                        "type": self._classify_dissonance_type(
                                            prep, res
                                        ),
                                        "preparation": prep,
                                        "resolution": res,
    ...
    def _find_simultaneous_notes(
        self, target_note: note.Note, score: stream.Score
    ) -> List[note.Note]:
    ...
    def _analyze_preparation(
        self, dissonant_note: note.Note, index: int, notes: List[note.Note]
    ) -> str:
