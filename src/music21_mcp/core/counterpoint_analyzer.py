...
class CounterpointAnalyzer:
    ...
    def __init__(self):
        self.consonant_intervals = {1, 3, 4, 5, 6, 8}  # Unison, 3rd, 4th, 5th, 6th, octave
    ...
    async def analyze_bach_chorale(
        self,
    ...
    async def analyze_fugue(
        ...
        if result.subject:
            # Find the answer
            result.answer = await self._find_fugue_answer(
            ...
            # Analyze key plan
            result.key_plan = await self._analyze_fugue_key_plan(
                fugue, result.entries
            )
    ...
    async def _analyze_first_species(
        self,
        cf_notes: List[note.Note],
        cp_notes: List[note.Note],
        ...
        for cf_index, cf_note in enumerate(cf_notes):
            ...
            for beat in range(1, 4):
                ...
                if cp_index < len(cp_notes):
                    # Check melodic flow
                    if cp_index >= 2:
                        leap = abs(cp_notes[cp_index].pitch.midi - 
    ...
    async def _analyze_fourth_species(
        ...
        for i in range(1, len(cp_notes)):
            ...
            if i < len(cf_notes):
                ...
                susp_interval = interval.Interval(
                    cf_notes[i].pitch, cp_notes[i].pitch
                )
    ...
    def _check_melodic_quality(
        ...
        for i in range(len(melody_notes) - 1):
            ...
            if leap == 6:
                result.errors.append({
                    'type': CounterpointError.TRITONE_LEAP,
                    'measure': i + 1
                })
            # Large leap (> octave)
            elif leap > 12:
            ...
            # Leap > 6th should be followed by stepwise motion in opposite direction
                ...
                if i + 2 < len(melody_notes):
                    next_motion = melody_notes[i+2].pitch.midi - melody_notes[i+1].pitch.midi
                    if (leap > 0 and next_motion > 2) or (leap < 0 and next_motion < -2):
                        result.warnings.append({
                            'type': 'leap_not_recovered',
                            'measure': i + 1
                        })
    ...
    def _check_cadences(
        ...
        if len(cf_notes) >= 2 and len(cp_notes) >= 2:
            # Check final cadence
            penult_interval = interval.Interval(
                cf_notes[-2].pitch, cp_notes[-2].pitch
            )
            final_interval = interval.Interval(
                cf_notes[-1].pitch, cp_notes[-1].pitch
            )
            
            # Common cadence formulas
            if cf_notes[-2].pitch < cf_notes[-1].pitch:  # CF ascending
                # Should have 6-8 or 3-1 in upper voice
                if not (penult_interval.generic.value == 6 and 
                       final_interval.generic.value in [1, 8]):
                    result.warnings.append({
                ...
                valid_cadence = False
                if (penult_interval.generic.value == 3 and 
                    final_interval.generic.value == 1):
                    valid_cadence = True
    ...
    def _is_valid_passing_tone(
        ...
        if (abs(prev_pitch - curr_pitch) <= 2 and 
    ...
    def _calculate_voice_leading_score(self, result: SpeciesAnalysis) -> float:
        ...
        parallel_errors = sum(1 for e in result.errors 
                            if e['type'] == CounterpointError.PARALLEL_PERFECT)
    ...
    def _calculate_melodic_score(self, result: SpeciesAnalysis) -> float:
        ...
        leap_errors = sum(1 for e in result.errors 
                         if e['type'] in [CounterpointError.LARGE_LEAP, 
                                         CounterpointError.TRITONE_LEAP])
        
        repetition_warnings = sum(1 for w in result.warnings
                                if w['type'] == CounterpointError.REPEATED_NOTE)
    ...
    def _calculate_harmonic_score(self, result: SpeciesAnalysis) -> float:
        ...
        perfect_count = sum(count for interval, count in result.interval_usage.items()
                          if 'P' in interval)
    ...
    def _extract_voices_from_chords(self, score: stream.Score) -> List[stream.Part]:
        ...
        for element in score.flatten():
            if isinstance(element, chord.Chord):
                ...
                if len(pitches) >= 4:
                    voices[3].append(note.Note(pitches[0], quarterLength=element.quarterLength))  # Bass
                    voices[2].append(note.Note(pitches[1], quarterLength=element.quarterLength))  # Tenor
                    voices[1].append(note.Note(pitches[2], quarterLength=element.quarterLength))  # Alto
                    voices[0].append(note.Note(pitches[3], quarterLength=element.quarterLength))  # Soprano
        
        return voices
    
    def _analyze_voice_ranges(self, parts: List[stream.Part]) -> Dict[str, Dict[str, Any]]:
                    ...
                    if pitches:
                        ...
                        if i < len(standard_ranges):
                            ...
                            if voice_range['lowest'] < std_low:
                                voice_range['below_standard'] = std_low - voice_range['lowest']
                            if voice_range['highest'] > std_high:
                                voice_range['above_standard'] = voice_range['highest'] - std_high
    ...
    def _align_voices_by_offset(
        self,
        notes1: List[note.Note],
        notes2: List[note.Note]
            ...
            if rhythm_patterns:
                ...
                for i in range(len(rhythm_patterns[0])):
                    if all(i < len(rp) for rp in rhythm_patterns):
                        if all(rp[i] == rhythm_patterns[0][i] for rp in rhythm_patterns):
                            matching_rhythms += 1
                        total_comparisons += 1
        ...
        for i in range(len(parts)):
            for j in range(i + 1, len(parts)):
                ...
                if rhythms1 and rhythms2:
                    ...
                    different_rhythms = sum(1 for k in range(min_len)
                                          if rhythms1[k] != rhythms2[k])
        ...
        for i in range(len(parts)):
            for j in range(i + 1, len(parts)):
                ...
                if intervals1 and intervals2:
                    ...
                    different_intervals = sum(1 for k in range(min_len)
                                            if intervals1[k] != intervals2[k])
        ...
        for i in range(len(contours)):
            for j in range(i + 1, len(contours)):
                if contours[i] and contours[j]:
                    ...
                    different = sum(1 for k in range(min_len)
                                  if contours[i][k] != contours[j][k])
            ...
            if notes1[i].isNote and notes2[i].isNote:
                ...
                if interval < 2:  # Less than a whole step
                    overlaps.append({
                        ...
                        'interval': interval,
                        'pitches': [notes1[i].nameWithOctave, notes2[i].nameWithOctave]
                    })
        
        return overlaps
    
    def _calculate_motion_statistics(self, parts: List[stream.Part]) -> Dict[str, float]:
                ...
                for k in range(len(aligned) - 1):
                    if (all(aligned[k]) and all(aligned[k + 1])):
                        motion = self._classify_motion(
                            aligned[k][0], aligned[k][1],
                            aligned[k + 1][0], aligned[k + 1][1]
                        )
    ...
    async def _find_fugue_subject(self, fugue: stream.Score) -> Optional[Dict[str, Any]]:
            ...
            for i in range(len(notes) - len(subject_intervals)):
                ...
                if all(n.isNote for n in candidate_notes):
                    ...
                    if candidate_intervals == subject_intervals:
                        transposition = candidate_notes[0].pitch.midi - subject['notes'][0].pitch.midi
    ...
    def _is_tonal_answer(
        ...
        differences = [abs(s - c) for s, c in zip(subject_intervals, candidate_intervals)]
        ...
        for i in range(len(entries) - 1):
            ...
            if end_measure > start_measure:
                episodes.append({
                    ...
                    'type': 'sequential' if (end_measure - start_measure) > 4 else 'link'
                })
