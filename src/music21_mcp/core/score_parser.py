...
class FormatDetector:
    ...
    CONTENT_PATTERNS = {
        'musicxml': re.compile(r'<(?:score-partwise|score-timewise|opus)', re.IGNORECASE),
        'abc': re.compile(r'^[XTMKLQVW]:', re.MULTILINE),
        'kern': re.compile(r'\*\*kern|\*\*dynam|\*\*text', re.MULTILINE),
        'lilypond': re.compile(r'\\(?:version|score|relative|new|header)', re.MULTILINE),
                            ...
                            with zipfile.ZipFile(io.BytesIO(content)) as zf:
                                for name in zf.namelist():
                                    if name.endswith('.xml'):
                                        ...
                                        if b'score-partwise' in xml_content or b'score-timewise' in xml_content:
                                            return 'musicxml'
                        except:
                            pass
                    else:
    ...
    async def parse(
        ...
        **options
        ...
        score = midi.translate.midiFileToStream(
            ...
            quarterLengthDivisors=options.get('quarter_length_divisors', [4, 3, 2, 1]) if quantize else None
        )
        ...
        for quantize in [True, False]:
            for divisors in [[4, 2, 1], [4, 3, 2, 1], [8, 6, 4, 3, 2, 1]]:
                try:
                    ...
                    return await self.parse(content, encoding, quantize=quantize, **options)
                except:
                    continue
