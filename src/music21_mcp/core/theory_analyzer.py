...
class TheoryAnalyzer:
    ...
    async def analyze_key(
        ...
        if window_size:
            local_keys = await self._analyze_local_keys(score, window_size, method)
            modulations = self._detect_modulations(local_keys, confidence_threshold)
    ...
    async def _detect_global_key(
        ...
        if len(score.parts) > 1:
            ...
            if key_votes:
                ...
                # Get alternatives
                sorted_keys = sorted(
                    key_votes.items(), key=lambda x: x[1], reverse=True
                )
                alternatives = [
    ...
    def _detect_modulations(
        self, local_keys: List[Dict[str, Any]], threshold: float
    ) -> List[Dict[str, Any]]:
    ...
    def _analyze_pitch_content(self, segment: stream.Stream) -> Dict[str, Any]:
        ...
        return {
            ...
            "pitch_class_distribution": {
                pc: count / total_pitches for pc, count in pitch_counts.items()
            },
        ...
        for scale_name, scale_class in scales_to_check:
            for tonic_pc in range(12):
                ...
                try:
                    ...
                    coverage = sum(1 for pc in scale_pcs if pc in pitch_classes) / len(
                        scale_pcs
                    )
    ...
    async def analyze_chord_quality(
        self, chord_obj: chord.Chord, include_extensions: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        if chord_obj.isTriad():
            analysis["triad_type"] = (
                ...
                if chord_obj.isMajorTriad()
                    ...
                    else "diminished" if chord_obj.isDiminishedTriad() else "augmented"
                )
            )
        ...
        for i in range(1, len(chords)):
            ...
            try:
                ...
                cadence_type = self._identify_cadence_type(
                    prev_rn.romanNumeral, curr_rn.romanNumeral
                )
    ...
    async def find_parallel_keys(
        self, reference_key: Union[key.Key, str]
    ) -> Dict[str, Any]:
