...
class ImportScoreTool(BaseTool):
    ...
    SUPPORTED_FILE_EXTENSIONS = {'.mid', '.midi', '.xml', '.musicxml', '.mxl', '.abc', '.krn', '.mei'}
    
    async def execute(self, score_id: str, source: str, source_type: Optional[str] = "auto") -> Dict[str, Any]:
        """
        ...
        """
        # Validate inputs
        error = self.validate_inputs(score_id=score_id, source=source, source_type=source_type)
        if error:
    ...
    def validate_inputs(self, score_id: str, source: str, source_type: str) -> Optional[str]:
    ...
    def _detect_source_type(self, source: str) -> str:
        ...
        if '/' in source and not os.path.exists(source):
            ...
            if any(composer in source.lower() for composer in ['bach', 'mozart', 'beethoven', 'chopin']):
                return "corpus"
            ...
            for i, note_str in enumerate(tokens):
                try:
                    ...
                    if i % 10 == 0:  # Update progress every 10 notes
                        self.report_progress(0.3 + (0.4 * i / total), f"Parsing note {i+1}/{total}")
                except Exception as e:
