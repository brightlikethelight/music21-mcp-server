...
class HarmonyAnalysisTool(BaseTool):
    ...
    async def execute(self, score_id: str, analysis_type: str = "comprehensive",
                     include_voice_leading: bool = True,
                     include_jazz_analysis: bool = False,
                     include_modulations: bool = True) -> Dict[str, Any]:
        """
        ...
        with self.error_handling(f"Harmonic analysis for '{score_id}'"):
            ...
            if analysis_type == "comprehensive":
                return await self._comprehensive_analysis(
                    score, include_voice_leading, include_jazz_analysis, include_modulations
                )
                ...
                return self.create_error_response(f"Unknown analysis type: {analysis_type}")
    
    def validate_inputs(self, score_id: str, analysis_type: str, **kwargs) -> Optional[str]:
    ...
    async def _comprehensive_analysis(self, score: stream.Score, 
                                    include_voice_leading: bool,
                                    include_jazz: bool,
                                    include_modulations: bool) -> Dict[str, Any]:
        ...
        try:
            ...
            results['functional_harmony'] = self._serialize_functional_analysis(functional_result)
            ...
            if include_modulations:
                ...
                results['modulations'] = self._serialize_modulation_analysis(modulation_result)
    ...
    async def _roman_numeral_analysis(self, score: stream.Score) -> Dict[str, Any]:
        ...
        try:
            ...
            for i, ch in enumerate(chord_list):
                if i % 10 == 0:
                    self.report_progress(0.6 + (0.3 * i / len(chord_list)), 
                ...
                try:
                    ...
                    roman_numerals.append({
                        ...
                        'confidence': rn.commonName != 'Chord' if hasattr(rn, 'commonName') else True
                    })
                except Exception as e:
                    logger.warning(f"Could not analyze chord at offset {ch.offset}: {e}")
                    roman_numerals.append({
                        'roman': '?',
                        'offset': float(ch.offset),
                        'duration': float(ch.duration.quarterLength),
    ...
    async def _cadence_analysis(self, score: stream.Score) -> Dict[str, Any]:
        ...
        try:
            ...
            cadence_density = total_cadences / max(1, score.duration.quarterLength) * 16  # Per 16 quarters
    ...
    def _serialize_voice_leading(self, analysis) -> Dict[str, Any]:
        ...
        return {
            'errors': [{
                ...
                'voices': e.get('voices', [])
            } for e in analysis.errors[:20]],  # Limit
    ...
    def _identify_common_progressions(self, roman_numerals: List[str]) -> List[Dict[str, Any]]:
        ...
        for name, pattern in common_patterns.items():
            ...
            for i in range(len(roman_numerals) - pattern_length + 1):
                ...
                if all(self._roman_match(segment[j], pattern[j]) for j in range(pattern_length)):
    ...
    def _summarize_progression(self, roman_numerals: List[str]) -> Dict[str, Any]:
        ...
        tonic_count = sum(1 for rn in roman_numerals if 'I' in rn and 'V' not in rn and 'I' not in rn)
        dominant_count = sum(1 for rn in roman_numerals if 'V' in rn)
        
        return {
            'unique_chords': len(chord_counts),
            'most_common': [{'chord': chord, 'count': count} for chord, count in most_common],
