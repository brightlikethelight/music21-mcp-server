...
from music21 import chord, roman, stream, key
...
class HarmonyAnalysisTool(BaseTool):
    ...
    def _analyze_roman_numerals(self, chords: List[chord.Chord], score: stream.Score) -> List[Dict[str, Any]]:
    ...
    def _analyze_progressions(self, roman_numerals: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _analyze_functional_harmony(self, roman_numerals: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def _analyze_harmonic_rhythm(self, chords: List[chord.Chord]) -> List[Dict[str, Any]]:
        """Analyze harmonic rhythm"""
        rhythm = []
        
        for i, chord_obj in enumerate(chords):
            duration = float(chord_obj.quarterLength) if hasattr(chord_obj, 'quarterLength') else 1.0
            rhythm.append({
                'position': i,
                'duration': duration,
                'chord': chord_obj.pitchedCommonName
