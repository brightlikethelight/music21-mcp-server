...
class VoiceLeadingAnalysisTool(BaseTool):
    ...
    async def execute(self, score_id: str, 
                     ...
                     educational_mode: bool = True) -> Dict[str, Any]:
        """
        ...
        with self.error_handling(f"Voice leading analysis for '{score_id}'"):
            ...
            if include_suggestions:
                ...
                suggestions = self._generate_suggestions(errors_by_type, motion_analysis)
            ...
            result = self.create_success_response(
                ...
                errors_by_type=self._format_errors_by_type(errors_by_type, educational_mode),
            ...
            if style == "strict":
                result['counterpoint_assessment'] = self._assess_counterpoint_quality(result)
        ...
        try:
            ...
            for i in range(min(len(soprano), len(bass)) - 1):
                try:
                    ...
                    int2 = interval.Interval(noteStart=bass[i+1], noteEnd=soprano[i+1])
                    ...
                    elif (sop_motion > 0 and bass_motion < 0) or (sop_motion < 0 and bass_motion > 0):
        ...
        try:
            ...
            for part in score.parts:
                ...
                for i in range(len(notes) - 1):
                    ...
                    if hasattr(current, 'pitch') and current.pitch.name == leading_tone_pitch.name:
            ...
            for category, counts in resolutions.items():
                ...
                if total > 0:
                    resolution_rates[category] = round(100 * counts['resolved'] / total, 1)
                else:
    ...
    def _format_errors_by_type(self, errors_by_type: Dict, educational: bool) -> List[Dict]:
        ...
        for error_type, errors in errors_by_type.items():
            error_info = {
                ...
                'locations': [self._format_error_location(e) for e in errors[:5]]  # Limit
            }
    ...
    def _generate_suggestions(self, errors_by_type: Dict, motion_analysis: Dict) -> List[Dict]:
    ...
    def _assess_counterpoint_quality(self, analysis_result: Dict) -> Dict[str, Any]:
        ...
        if score >= 90:
            ...
            assessment['grade'] = 'D'
            assessment['overall_comment'] = 'Poor counterpoint needing significant improvement'
        else:
            assessment['grade'] = 'F'
            assessment['overall_comment'] = 'Counterpoint has major issues requiring revision'
