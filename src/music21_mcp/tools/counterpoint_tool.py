...
class CounterpointGeneratorTool(BaseTool):
    ...
    async def execute(self, score_id: str,
        ...
        """
        # Validate inputs
        error = self.validate_inputs(score_id=score_id, species=species, 
                                   voice_position=voice_position, rule_set=rule_set)
        if error:
        ...
        with self.error_handling(f"Counterpoint generation for '{score_id}'"):
            ...
            rule_report = self._check_counterpoint_rules(
                cantus_firmus, counterpoint['notes'], 
                species_enum, rule_set, custom_rules
            )
    ...
    def validate_inputs(self, score_id: str, species: str, 
                       voice_position: str, rule_set: str, **kwargs) -> Optional[str]:
        ...
        if species not in valid_species:
            return f"Invalid species: {species}. Choose from: {', '.join(valid_species)}"
    ...
    async def _generate_first_species(self, cantus_firmus: List[note.Note],
                                    cf_key: m21_key.Key,
                                    voice_position: str,
                                    rule_set: str,
                                    custom_rules: Optional[List[str]]) -> Dict[str, Any]:
        ...
        for i, cf_note in enumerate(cantus_firmus):
            self.report_progress(0.3 + (0.4 * i / len(cantus_firmus)), 
            ...
            if i == 0:
                ...
                cp_pitch = self._choose_first_species_note(
                    ...
                    counterpoint_notes, cantus_firmus, i,
    ...
    def _choose_first_species_note(self, cf_note: note.Note, cf_key: m21_key.Key,
                                 voice_position: str, previous_notes: List[note.Note],
                                 cantus_firmus: List[note.Note], cf_index: int,
                                 rule_set: str, custom_rules: Optional[List[str]]) -> pitch.Pitch:
        """Choose appropriate note for first species counterpoint"""
        # Get available pitches in key
        available_pitches = self._get_scale_pitches(cf_key, cf_note.pitch, voice_position)
    ...
    def _get_scale_pitches(self, key: m21_key.Key, reference_pitch: pitch.Pitch, 
                          voice_position: str) -> List[pitch.Pitch]:
    ...
    async def _generate_second_species(self, cantus_firmus: List[note.Note],
                                     cf_key: m21_key.Key,
                                     voice_position: str,
                                     rule_set: str,
                                     custom_rules: Optional[List[str]]) -> Dict[str, Any]:
        ...
        for i, cf_note in enumerate(cantus_firmus):
            ...
            for beat in range(2):
                ...
                if i == 0 and beat == 0:
                    ...
                    cp_pitch = self._choose_penultimate_pitch(cf_note, cf_key, voice_position)
                    ...
                    cp_pitch = self._choose_second_species_note(
                        cf_note, cf_key, voice_position,
                        counterpoint_notes, is_strong_beat,
                        rule_set, custom_rules
                    )
                
                # Create note with half duration
                cp_note = note.Note(cp_pitch, quarterLength=cf_note.duration.quarterLength / 2)
                counterpoint_notes.append(cp_note)
    ...
    def _choose_second_species_note(self, cf_note: note.Note, cf_key: m21_key.Key,
                                  voice_position: str, previous_notes: List[note.Note],
                                  is_strong_beat: bool, rule_set: str,
                                  custom_rules: Optional[List[str]]) -> pitch.Pitch:
        ...
        if previous_notes:
            ...
            # Prefer stepwise motion
            stepwise_candidates = [p for p in candidates 
                                 if abs(p.midi - prev_pitch.midi) <= 2]
    ...
    async def _generate_third_species(self, cantus_firmus: List[note.Note],
                                    cf_key: m21_key.Key,
                                    voice_position: str,
                                    rule_set: str,
                                    custom_rules: Optional[List[str]]) -> Dict[str, Any]:
            ...
            for beat in range(3):
                ...
                if i == 0 and beat == 0:
                    ...
                    cp_pitch = self._choose_third_species_note(
                        cf_note, cf_key, voice_position,
                        counterpoint_notes, beat, is_strong,
                        rule_set, custom_rules
                    )
                
                cp_note = note.Note(cp_pitch, quarterLength=cf_note.duration.quarterLength / 3)
                counterpoint_notes.append(cp_note)
    ...
    def _choose_third_species_note(self, cf_note: note.Note, cf_key: m21_key.Key,
                                 voice_position: str, previous_notes: List[note.Note],
                                 beat: int, is_strong: bool, rule_set: str,
                                 custom_rules: Optional[List[str]]) -> pitch.Pitch:
        ...
        if is_strong:
            ...
            candidates = [p for p in available_pitches
            ...
            if previous_notes and beat == 1:
                ...
                prev_pitch = previous_notes[-1].pitch
                neighbors = [p for p in candidates
                           if abs(p.midi - prev_pitch.midi) == 1]
    ...
    async def _generate_fourth_species(self, cantus_firmus: List[note.Note],
                                     cf_key: m21_key.Key,
                                     voice_position: str,
                                     rule_set: str,
                                     custom_rules: Optional[List[str]]) -> Dict[str, Any]:
        ...
        for i, cf_note in enumerate(cantus_firmus):
            if i == 0:
                ...
                suspension_notes = self._create_suspension(
                    cf_note, cantus_firmus[i-1] if i > 0 else None,
                    cf_key, voice_position, counterpoint_notes,
                    rule_set
                )
    ...
    def _create_suspension(self, cf_note: note.Note, prev_cf: Optional[note.Note],
                         cf_key: m21_key.Key, voice_position: str,
                         previous_notes: List[note.Note], rule_set: str) -> List[note.Note]:
        ...
        suspension_note = note.Note(prep_pitch, quarterLength=cf_note.duration.quarterLength / 2)
        ...
        while abs(resolution_pitch.midi - cf_note.pitch.midi) % 12 not in self.consonances:
            resolution_pitch.midi -= 1
            if resolution_pitch.midi < cf_note.pitch.midi - 12:
                # Fallback to consonant interval
                resolution_pitch = self._choose_consonant_pitch(cf_note, cf_key, voice_position)
                break
        
        resolution_note = note.Note(resolution_pitch, quarterLength=cf_note.duration.quarterLength / 2)
        notes.append(resolution_note)
        
        return notes
    
    def _choose_consonant_pitch(self, cf_note: note.Note, cf_key: m21_key.Key,
                               voice_position: str) -> pitch.Pitch:
        """Choose a consonant pitch above or below CF note"""
        available = self._get_scale_pitches(cf_key, cf_note.pitch, voice_position)
        consonant = [p for p in available 
                    if abs(p.midi - cf_note.pitch.midi) % 12 in self.consonances]
        
        if consonant:
            # Prefer imperfect consonances
            imperfect = [p for p in consonant
    ...
    async def _generate_fifth_species(self, cantus_firmus: List[note.Note],
                                    cf_key: m21_key.Key,
                                    voice_position: str,
                                    rule_set: str,
                                    custom_rules: Optional[List[str]]) -> Dict[str, Any]:
        ...
        for i, cf_note in enumerate(cantus_firmus):
            ...
            if i == 0:
                ...
                species_choice = random.choice([
                    Species.FIRST, Species.SECOND, Species.SECOND,  # Weight towards 2:1
                    Species.THIRD, Species.FOURTH
                ])
    ...
    def _choose_opening_pitch(self, cf_note: note.Note, voice_position: str) -> pitch.Pitch:
    ...
    def _choose_penultimate_pitch(self, cf_note: note.Note, cf_key: m21_key.Key,
                                 voice_position: str) -> pitch.Pitch:
    ...
    def _choose_final_pitch(self, cf_note: note.Note, voice_position: str) -> pitch.Pitch:
    ...
    def _analyze_melodic_line(self, notes: List[note.Note]) -> Dict[str, Any]:
        ...
        for i in range(len(pitched_notes) - 1):
            interval_size = abs(pitched_notes[i+1].pitch.midi - pitched_notes[i].pitch.midi)
            ...
            # Direction changes
            current_direction = pitched_notes[i+1].pitch.midi - pitched_notes[i].pitch.midi
            if current_direction != 0:
                if prev_direction != 0 and (current_direction > 0) != (prev_direction > 0):
                    direction_changes += 1
                prev_direction = current_direction
        
        analysis['stepwise_motion_percentage'] = (stepwise / (len(pitched_notes) - 1)) * 100
    ...
    def _check_counterpoint_rules(self, cantus_firmus: List[note.Note],
                                counterpoint: List[note.Note],
                                species: Species, rule_set: str,
                                custom_rules: Optional[List[str]]) -> Dict[str, Any]:
    ...
    def _check_parallel_intervals(self, cantus_firmus: List[note.Note],
                                counterpoint: List[note.Note],
                                interval_size: int) -> List[int]:
        ...
        for i in range(min(len(cf_notes), len(cp_notes)) - 1):
            ...
            if i + 1 < min(len(cf_notes), len(cp_notes)):
                next_interval = (cp_notes[i+1].pitch.midi - cf_notes[i+1].pitch.midi) % 12
                ...
                if curr_interval == interval_size and next_interval == interval_size:
                    ...
                    if cf_motion != 0 and cp_motion != 0 and (cf_motion > 0) == (cp_motion > 0):
                        locations.append(i)
        
        return locations
    
    def _check_cadence(self, cantus_firmus: List[note.Note],
                     counterpoint: List[note.Note]) -> bool:
    ...
    def _create_counterpoint_score(self, cantus_firmus: List[note.Note],
                                 counterpoint: List[note.Note],
                                 voice_position: str,
                                 species: Species) -> stream.Score:
    ...
    def _generate_explanations(self, species: Species, 
                             rule_report: Dict[str, Any]) -> List[Dict[str, str]]:
