...
class PatternRecognitionTool(BaseTool):
    ...
    async def execute(self, score_id: str, 
                     ...
                     include_transformations: bool = True) -> Dict[str, Any]:
        """
        ...
        with self.error_handling(f"Pattern recognition for '{score_id}'"):
            ...
            if pattern_type in ["melodic", "both"]:
                ...
                melodic_results = await self._find_melodic_patterns(
                    score, min_pattern_length, similarity_threshold, include_transformations
                )
    ...
    def validate_inputs(self, score_id: str, pattern_type: str, 
                       similarity_threshold: float, **kwargs) -> Optional[str]:
    ...
    async def _find_melodic_patterns(self, score: stream.Score, 
                                   min_length: int, threshold: float,
                                   include_transformations: bool) -> Dict[str, Any]:
        ...
        try:
            ...
            for melody in melodies:
                motifs = self._find_motifs(melody, min_length, threshold, include_transformations)
    ...
    async def _find_rhythmic_patterns(self, score: stream.Score,
                                    min_length: int, threshold: float) -> Dict[str, Any]:
        ...
        try:
            ...
            # Deduplicate
            results['rhythmic_motifs'] = self._deduplicate_patterns(results['rhythmic_motifs'])[:15]
            
            # Add summary
            results['summary'] = {
                'total_motifs': len(results['rhythmic_motifs']),
                'syncopation_density': len(results['syncopations']) / max(1, score.duration.quarterLength),
            ...
            for start in range(len(intervals) - pattern_length * 2 + 1):
                ...
                for next_start in range(start + pattern_length, len(intervals) - pattern_length + 1):
                    if intervals[next_start:next_start + pattern_length] == pattern:
                        ...
                        transposition = melody[next_start].pitch.midi - melody[start].pitch.midi
                        
                        sequences.append({
                            'type': 'real_sequence' if transposition != 0 else 'literal_repeat',
    ...
    def _find_motifs(self, melody: List, min_length: int, 
                    threshold: float, include_transformations: bool) -> List[Dict]:
        ...
        for i in range(len(melody) - min_length + 1):
            ...
            pitch_sequences.append({
                'pitches': pitches,
                'start': i,
                'measure': melody[i].measureNumber
            })
        ...
        for i, seq1 in enumerate(pitch_sequences):
            ...
            if len(similar) >= 2:  # At least 2 occurrences
                motifs.append({
                    ...
                    'instances': [{'measure': s['measure'], 'position': s['start']} for s in similar],
                    'transformations': self._identify_transformations(similar) if include_transformations else []
                })
                processed.add(i)
    ...
    def _calculate_pitch_similarity(self, seq1: List[int], seq2: List[int], 
                                  include_transformations: bool) -> float:
        ...
        for pattern, count in pattern_counts.items():
            if count >= 2:
                patterns.append({
                    ...
                    'musical_significance': self._assess_interval_pattern_significance(pattern)
                })
        
        # Sort by significance and frequency
        patterns.sort(key=lambda x: (x['occurrences'], x['musical_significance']), reverse=True)
    ...
    def _find_rhythmic_motifs(self, rhythm_stream: List[Dict], 
                            min_length: int, threshold: float) -> List[Dict]:
    ...
    def _calculate_rhythmic_similarity(self, rhythm1: List[float], 
                                     rhythm2: List[float]) -> float:
                ...
                if beat_strength < 0.5:  # Weak beat
                    ...
                    if end_beat > next_strong_beat:
                        syncopations.append({
                            ...
                            'pitch': str(note.pitch) if hasattr(note, 'pitch') else 'rest',
        ...
        try:
            ...
            for part in score.parts[:1]:  # Analyze first part
                ...
                for i, element in enumerate(elements):
                    ...
                    if (element.isRest and element.duration.quarterLength >= 1.0) or i == len(elements) - 1:
                        if i > current_phrase_start:
                            phrase_length = sum(e.duration.quarterLength 
                                              for e in elements[current_phrase_start:i])
                            
                            structure['phrases'].append({
                                'start_measure': elements[current_phrase_start].measureNumber,
                                'end_measure': elements[i-1].measureNumber if i > 0 else 1,
                                'length': phrase_length,
                                'note_count': sum(1 for e in elements[current_phrase_start:i] if e.isNote)
                            })
            ...
            if len(structure['phrase_lengths']) >= 2:
                ...
                elif len(lengths) == 4 and lengths[0] == lengths[2] and lengths[1] == lengths[3]:
                    structure['symmetry'] = 'AABB'
                elif len(lengths) == 4 and lengths[0] == lengths[3] and lengths[1] == lengths[2]:
    ...
    def _get_common_intervals(self, melodies: List[List]) -> List[Dict]:
        ...
        return [{'interval': name, 'count': count} for name, count in sorted_intervals[:10]]
    
    def _calculate_melodic_density(self, melodies: List[List], score: stream.Score) -> float:
    ...
    def _identify_transformations(self, similar_patterns: List[Dict]) -> List[str]:
        ...
        for pattern in similar_patterns[1:]:
            ...
            if len(pitches) == len(base_pitches):
                ...
                # Check for inversion
                if all(base_pitches[0] - (p - base_pitches[0]) == pitches[i] 
