...
class HarmonizationTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> Dict[str, Any]:
        ...
        with self.error_handling(f"Harmonization of '{score_id}'"):
            ...
            voice_leading_check = self._check_voice_leading(
                harmonization["score"], self.style_vocabularies[style]["voice_leading"]
            )
    ...
    def _is_chord_compatible(
        self, chord_symbol: str, melodic_note: note.Note, key_obj
    ) -> bool:
    ...
    def _realize_chord_classical(
        self, chord_symbol: str, key_obj, melodic_note: note.Note
    ) -> List[pitch.Pitch]:
    ...
    def _create_pop_voicing(self, chord_symbol: str, key_obj) -> List[pitch.Pitch]:
        ...
        try:
            ...
            if (
                self.style_vocabularies["pop"].get("power_chords")
    ...
    def _create_modal_voicing(
        self, chord_symbol: str, key_obj, mode: str
    ) -> List[pitch.Pitch]:
    ...
    def _generate_explanations(
        ...
        if progression:
            ...
            for common_name, common_prog in self.common_progressions.get(
                style, {}
            ).items():
