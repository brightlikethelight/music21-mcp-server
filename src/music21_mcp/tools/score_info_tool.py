...
class ScoreInfoTool(BaseTool):
    ...
    async def execute(self, score_id: str, include_instruments: bool = True,
                     include_structure: bool = True) -> Dict[str, Any]:
        """
    ...
    def _extract_metadata(self, score: stream.Score) -> Dict[str, Any]:
        ...
        try:
            if hasattr(score, 'metadata'):
                ...
                # Additional metadata
                for attr in ['alternativeTitle', 'popularTitle', 'parentTitle', 
                           'groupTitle', 'localeOfComposition']:
        ...
        try:
            if hasattr(score, 'parts'):
                for i, part in enumerate(score.parts):
                    part_info = {
                        'part_number': i + 1,
                        'part_name': part.partName if hasattr(part, 'partName') else f"Part {i+1}"
                    }
    ...
    def _analyze_detailed_structure(self, score: stream.Score) -> Dict[str, Any]:
        ...
        try:
            ...
            if dynamics:
                ...
                structure['dynamic_range'] = list(set(d.value for d in dynamics if hasattr(d, 'value')))
