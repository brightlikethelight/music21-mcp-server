...
class RateLimiter:
    ...
    def _get_metadata(self, bucket: "TokenBucket", endpoint: str = None) -> dict[str, Any]:
        """Get rate limit metadata for response headers"""
        return {
            "limit": int(bucket.capacity),
            "remaining": int(bucket.tokens),
            "reset": int(time.time() + (bucket.capacity - bucket.tokens) / bucket.refill_rate),
            "retry_after": None if bucket.tokens > 0 else int((1 - bucket.tokens) / bucket.refill_rate),
            "endpoint": endpoint,
        }
...
def rate_limit(
    ...
    def decorator(func):
        async def wrapper(request: Request, *args, **kwargs):
            ...
            now = time.time()
            data = wrapper._rate_limit_data.get(identifier, {"count": 0, "reset": now + 60})
            ...
            if len(wrapper._rate_limit_data) > 1000:
                ...
                wrapper._rate_limit_data = {
                    k: v for k, v in wrapper._rate_limit_data.items()
                    if v["reset"] > now
                }
