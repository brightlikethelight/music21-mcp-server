    ...
    def _estimate_object_size(self, obj: Any) -> int:
        ...
        try:
            ...
            if hasattr(obj, 'flat') and hasattr(obj, 'notes'):
                ...
                note_count = len(list(obj.flat.notes)) if hasattr(obj.flat, 'notes') else 100
...
class ResourceManager:
    ...
    def check_health(self) -> Dict[str, Any]:
        ...
        if storage["memory_utilization_percent"] > 90:
            errors.append(f"Storage memory usage critical: {storage['memory_utilization_percent']:.1f}%")
        elif storage["memory_utilization_percent"] > 75:
            warnings.append(f"Storage memory usage high: {storage['memory_utilization_percent']:.1f}%")
        
        # Check system memory
        if system["process_memory_percent"] > 80:
            warnings.append(f"System memory usage high: {system['process_memory_percent']:.1f}%")
        
        # Check score count
        if storage["total_scores"] > self.max_scores * 0.9:
            warnings.append(f"Score count near limit: {storage['total_scores']}/{self.max_scores}")
