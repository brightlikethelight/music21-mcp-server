        ...
        with self._lock:
            ...
            if self.state == CircuitState.HALF_OPEN:
                ...
                if (self.failure_count >= self.config.failure_threshold or 
                    recent_failures >= self.config.max_failures_per_minute):
                    self.state = CircuitState.OPEN
                    self.next_attempt_time = time.time() + self.config.recovery_timeout
                ...
                if self.health_check:
                    try:
                        if not self.health_check(resource_wrapper['resource']):
                            logger.warning(f"Resource failed health check in pool {self.name}")
                            self._size -= 1
                            continue
                    except Exception as e:
        ...
        while self._monitoring:
            try:
                ...
                if current_mb > self.hard_limit_mb:
                    logger.critical(f"Memory usage critical: {current_mb:.0f}MB > {self.hard_limit_mb}MB")
                    await self._emergency_cleanup()
                
                elif current_mb > self.soft_limit_mb:
                    logger.warning(f"Memory usage high: {current_mb:.0f}MB > {self.soft_limit_mb}MB")
    ...
    async def cleanup(self):
        ...
        for resource in self.resources:
            try:
                ...
                logger.error(f"Error cleaning up resource in request {self.request_id}: {e}")
...
class AutoRecovery:
    ...
    async def attempt_recovery(self, error: Exception, context: Dict[str, Any]) -> bool:
        ...
        recent_recoveries = [
            ...
            if r['error_type'] == error_type and 
        ...
        try:
            ...
            if asyncio.iscoroutinefunction(strategy_info['strategy']):
                ...
                success = await asyncio.to_thread(strategy_info['strategy'], error, context)
...
# Recovery strategies
async def recover_from_connection_error(error: Exception, context: Dict[str, Any]) -> bool:
...
auto_recovery.register_strategy(OSError, recover_from_connection_error)  # File descriptor exhaustion
