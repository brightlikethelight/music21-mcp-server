...
) -> tuple[OAuth2Storage, SessionStorage]:
...
async def setup_demo_data(
    oauth2_storage: OAuth2Storage, oauth2_provider: OAuth2Provider
):
    ...
    logger.info(
        f"Server client created: {server_client.client_id} / {server_client.client_secret}"
    )
...
async def lifespan(app: FastAPI):
    ...
    session_config = SessionConfig(
        session_ttl_minutes=config.session_ttl_minutes, enable_sliding_expiration=True
    )
...
def create_remote_app(config: Optional[RemoteMCPConfig] = None) -> FastAPI:
    ...
    async def root():
        return {
            ...
            "mcp": {"tools_endpoint": "/mcp/tools", "execute_endpoint": "/mcp/execute"},
        }
    ...
    async def list_tools(auth_context: MCPAuthContext, request: Request):
        ...
        authenticated_server: AuthenticatedMCPServer = (
            request.app.state.authenticated_server
        )
    ...
    async def execute_tool(
        tool_name: str, arguments: dict, auth_context: MCPAuthContext, request: Request
    ):
    ...
    async def create_mcp_session(auth_context: MCPAuthContext, request: Request):
        ...
        return {"session_id": session_id, "expires_in": 3600}  # 1 hour

    @app.post("/mcp/session/{session_id}/execute/{tool_name}")
    async def execute_in_session(
        session_id: str, tool_name: str, arguments: dict, request: Request
    ):
    ...
    async def health_check():
        return {"status": "healthy", "service": "music21-mcp-remote"}
