...
class AuthenticatedMCPServer:
    ...
    async def check_tool_access(
        self, tool_name: str, auth_context: MCPAuthContext
    ) -> bool:
        ...
        if not auth_context.has_scope(required_scope):
            logger.warning(
                f"Access denied to {tool_name}: missing scope {required_scope}"
            )
            return False
    ...
    async def execute_tool(
        self, tool_name: str, arguments: Dict[str, Any], auth_context: MCPAuthContext
    ) -> Any:
...
class MCPSessionManager:
    ...
    async def create_mcp_session(self, session_id: str, auth_context: MCPAuthContext):
        """Create MCP session with auth context"""
        self.active_sessions[session_id] = auth_context
        logger.info(
            f"Created MCP session: {session_id} for user {auth_context.username}"
        )
...
async def validate_mcp_token(
    token: str, oauth2_storage: OAuth2Storage, required_scope: str = "read"
) -> MCPAuthContext:
