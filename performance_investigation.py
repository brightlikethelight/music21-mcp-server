...
class PerformanceInvestigator:
    ...
    async def time_operation(self, operation_name: str, operation_func, iterations: int = 10):
    ...
    async def test_import_operations(self):
        ...
        async def import_bach_chorale():
            return await self.service.import_score("perf_test_bach", "bach/bwv66.6", "corpus")
        
        bach_stats = await self.time_operation("Import Bach Chorale (Corpus)", import_bach_chorale, 5)
        self.results["import_bach_chorale"] = bach_stats

        # Test text notation import
        async def import_text_melody():
            return await self.service.import_score("perf_test_melody", "C4 E4 G4 C5 B4 G4 E4 C4", "text")
        
        text_stats = await self.time_operation("Import Text Melody", import_text_melody, 10)
    ...
    async def test_analysis_operations(self):
        ...
        harmony_stats = await self.time_operation("Harmony Analysis", analyze_harmony, 5)
    ...
    async def test_resource_operations(self):
        ...
        resource_stats = await self.time_operation("Get Resource Stats", get_resource_stats, 20)
        ...
        perf_stats = await self.time_operation("Get Performance Metrics", get_performance_metrics, 20)
    ...
    async def test_bulk_operations(self):
        ...
        bulk_import_stats = await self.time_operation("Bulk Import (5 scores)", import_multiple_scores, 3)
        ...
        bulk_analysis_stats = await self.time_operation("Bulk Key Analysis", analyze_all_keys, 3)
    ...
    def search_for_performance_claims(self):
        ...
        for doc_file in doc_files:
            ...
            if file_path.exists():
                ...
                performance_terms = [
                    "sub-200ms", "200ms", "milliseconds", "ms", "fast", "speed",
                    "performance", "benchmark", "latency", "response time"
                ]
    ...
    def analyze_results(self):
        ...
        for op_name, stats in self.results.items():
            if stats["status"] == "SUCCESS":
                ...
                status = "✅ FAST" if under_200_pct >= 90 else "⚠️ SLOW" if under_200_pct >= 50 else "❌ VERY SLOW"
        ...
        if all_operations:
            ...
            under_200_overall = sum(1 for t in all_operations if t < 200) / len(all_operations) * 100
        ...
        slow_ops = [(name, stats) for name, stats in self.results.items() 
        ...
        variable_ops = [(name, stats) for name, stats in self.results.items()
