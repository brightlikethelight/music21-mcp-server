...
def check_python_version():
    ...
    if version.major != 3 or version.minor < 10:
        ...
        return False
    log(f"Python version {version.major}.{version.minor}.{version.micro} is compatible", "INFO")
    return True
...
def check_disk_space(required_mb=500):
    ...
    try:
        ...
        if free_mb < required_mb:
            log(f"Insufficient disk space: {free_mb:.1f}MB available, {required_mb}MB required", "ERROR")
            return False
...
def install_dependencies(pip_exe: Path, requirements_file: Path):
    ...
    try:
        ...
        if process.returncode != 0:
            log(f"Failed to install dependencies (exit code: {process.returncode})", "ERROR")
            return False
