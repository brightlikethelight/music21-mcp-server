...
def get_claude_desktop_config_path():
    ...
    if system == "Darwin":  # macOS
        ...
        if appdata:
            ...
            # Fallback
            return Path.home() / "AppData" / "Roaming" / "claude-desktop" / "config.json"
...
def configure_claude_desktop(extension_dir: Path, env_info: dict):
    ...
    music21_config = {
        ...
        "env": {
            "PYTHONPATH": str(extension_dir / "src")
        }
    }
...
def main():
    ...
    if not env_file.exists():
        log("Environment info file not found - pre-installation may have failed", "ERROR")
        sys.exit(1)
    ...
    if success:
        ...
        log("The extension may still work, but manual configuration might be needed", "WARNING")
